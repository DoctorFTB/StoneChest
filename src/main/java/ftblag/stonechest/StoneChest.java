...
public class StoneChest {
    ...
    public StoneChest() {
        ...
        for (EnumStoneChest type : EnumStoneChest.values()) {
            parts[type.ordinal()] = new Item(new Item.Properties().tab(ItemGroup.TAB_MATERIALS)).setRegistryName(MODID, "part_" + type.name().toLowerCase());
            chests[type.ordinal()] = new BlockStoneChest(type);
        }

        chestTileType = TileEntityType.Builder.of(TileEntityStoneChest::new, chests).build(null);
